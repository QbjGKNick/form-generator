...
export function dialogWrapper(str) {
  return `<c-dialog v-bind="$attrs" v-on="$listeners" @open="onOpen" @close="onClose" title="Dialog Titile">
    ${str}
    <div slot="footer">
      <c-button @click="close">取消</c-button>
      <c-button type="primary" @click="handelConfirm">确定</c-button>
    </div>
  </c-dialog>`;
}
...
function buildFormTemplate(scheme, child, type) {
  ...
  let str = `<c-form ref="${scheme.formRef}" :model="${
    scheme.formModel
  ...
  if (someSpanIsNot24) {
    str = `<c-row :gutter="${scheme.gutter}">
        ${str}
      </c-row>`;
  }
  return str;
...
function colWrapper(scheme, str) {
  if (someSpanIsNot24 || scheme.__config__.span !== 24) {
    return `<c-col :span="${scheme.__config__.span}">
      ${str}
    </c-col>`;
  }
  return str;
...
const layouts = {
  colFormItem(scheme) {
    ...
    let str = `<c-form-item ${labelWidth} ${label} prop="${scheme.__vModel__}" ${required}>
        ${tagDom}
      </c-form-item>`;
  ...
  rowFormItem(scheme) {
    ...
    let str = `<c-row ${type} ${justify} ${align} ${gutter}>
      ${children.join("\n")}
    </c-row>`;
    str = colWrapper(scheme, str);
    return str;
...
function buildElSelectChild(scheme) {
  ...
  if (slot && slot.options && slot.options.length) {
    children.push(
      `<c-option v-for="(item, index) in ${scheme.__vModel__}Options" :key="index" :label="item.label" :value="item.value" :disabled="item.disabled"></c-option>`
    );
  }
  return children.join("\n");
}
...
function buildElUploadChild(scheme) {
  ...
  if (scheme["list-type"] === "picture-card")
    ...
    list.push(
      `<c-button size="small" type="primary" icon="el-icon-upload">${config.buttonText}</c-button>`
    );
  if (config.showTip)
    list.push(
